...
class ETLDiskJob(ProcessData):
    ...
    def perform_classification(self, processed_df, bucket_name = Optional[str]):
        if self.task:
            processed_df = self.run_classification(processed_df=processed_df, bucket_name=bucket_name)
        return processed_df
                ...
                if html_content:
                    ...
                    if not ProcessData.remove_text(text) and not self.maybe_check_bloom(text):
                        ...
                        if "ebay" in domain:
                            ...
                            self.add_information_to_metadata(domain, extract_dict, soup)
